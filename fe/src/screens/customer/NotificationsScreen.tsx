    ...
    <SafeAreaView edges={['top']} style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.headerAction} activeOpacity={0.85} onPress={markAllAsRead}>
          <MaterialCommunityIcons name="dots-horizontal-circle-outline" size={24} color={theme.colors.text} />
        </TouchableOpacity>
      </View>
            ...
            <View style={styles.cardContent}>
              <View style={styles.cardHeader}>
                <Text style={styles.title} numberOfLines={1}>
                  {item.title} {item.movieTitle ? <Text style={styles.movieText}>{item.movieTitle}</Text> : null}
                </Text>
                <Text style={styles.timeText}>{formatRelativeTime(item.createdAt)}</Text>
              </View>
              <Text style={[styles.message, item.type === 'COMMENT' && styles.commentText]} numberOfLines={2}>
                {item.message}
              </Text>
            </View>
